<!-- @generated by `cargo xtask validate-model --write` from model/errors.toml and model/browser-diagnostics.toml. -->
<!-- Do not edit. R1: the model is the single source. -->

# ERRORS

The closed CLI diagnostic registry uses PP codes; the browser journal has a separate typed namespace below.

## `PP1001` --- Unknown configuration field

The project configuration contains an unrecognized key.

Class: `configuration`. Exit code: 2.

## `PP1002` --- Missing required configuration field

A required project configuration field is absent.

Class: `configuration`. Exit code: 2.

## `PP1003` --- Invalid project limits

A configured resource limit is non-positive or exceeds maximum allowable threshold.

Class: `configuration`. Exit code: 2.

## `PP2001` --- Unresolved document reference

A model references a declaration or component that is not defined.

Class: `semantic`. Exit code: 1.

## `PP2002` --- Ambiguous facet term

A term is defined across multiple facet packages without explicit namespace qualification.

Class: `semantic`. Exit code: 1.

## `PP2003` --- Duplicate component identifier

Multiple components share the same qualified identifier.

Class: `semantic`. Exit code: 1.

## `PP2004` --- Dangling component edge

An edge endpoint references a non-existent component.

Class: `semantic`. Exit code: 1.

## `PP2005` --- Unresolved risk link

A risk definition references an undefined asset or threat.

Class: `semantic`. Exit code: 1.

## `PP2006` --- Unsatisfied security control

A required security control is not satisfied by the target architecture.

Class: `semantic`. Exit code: 1.

## `PP2007` --- Invalid quality requirement mapping

A quality requirement references an undefined ISO 25010 characteristic or measure.

Class: `semantic`. Exit code: 1.

## `PP2008` --- Cyclic lexicon import

A circular dependency was detected in facet lexicon package imports.

Class: `semantic`. Exit code: 1.

## `PP2009` --- Invalid text application profile

A text application violates its closed profile, metadata, byte bounds, exported roots, View, or acceptance-vector contract.

Class: `semantic`. Exit code: 1.

## `PP3001` --- Malformed Hologram archive header or version

A .holo file is not a binary Hologram v4 archive with the exact HOLO header.

Class: `holo`. Exit code: 1.

## `PP3002` --- Malformed Hologram section table

A Hologram section table has invalid offsets, lengths, alignment, ordering, or bounds.

Class: `holo`. Exit code: 1.

## `PP3003` --- Invalid Hologram section closure

A Holo/1 archive has a missing, duplicate, unknown, or noncanonically ordered section.

Class: `holo`. Exit code: 1.

## `PP3004` --- Hologram archive footer mismatch

The BLAKE3 footer does not authenticate the exact archive prefix.

Class: `holo`. Exit code: 1.

## `PP3005` --- Application manifest disagreement

The canonical AppManifest is malformed or disagrees with the modeled topology.

Class: `holo`. Exit code: 1.

## `PP3006` --- Capability request disagreement

The canonical capability request or child delegation disagrees with the model.

Class: `holo`. Exit code: 1.

## `PP3007` --- Hologram identity disagreement

A content, application, footer, or archive identity does not match its canonical preimage.

Class: `holo`. Exit code: 1.

## `PP3008` --- Content blob disagreement

A content blob is duplicated, mislabeled, malformed, or does not match its content kappa.

Class: `holo`. Exit code: 1.

## `PP3009` --- Incomplete fat archive

A fat archive omits a content or capability object required by its manifest.

Class: `holo`. Exit code: 1.

## `PP3010` --- Invalid modeled application layer

A modeled guest, View, or embedded model layer is malformed or unsupported.

Class: `holo`. Exit code: 1.

## `PP3011` --- Holo application projection disagreement

The projected Hologram application differs from the authoritative Prism application value.

Class: `holo`. Exit code: 1.

## `PP3012` --- Application directory disagreement

The application-directory extension does not exactly derive from the manifest and content closure.

Class: `holo`. Exit code: 1.

## `PP3013` --- Extension disagreement

A required extension is missing, duplicated, malformed, unknown, or noncanonical.

Class: `holo`. Exit code: 1.

## `PP3014` --- Source manifest disagreement

The derived source manifest or Metadata section disagrees with generated application artifacts.

Class: `holo`. Exit code: 1.

## `PP3015` --- Prism provenance disagreement

The closed Prism model-provenance extension is malformed or disagrees with embedded evidence.

Class: `holo`. Exit code: 1.

## `PP4001` --- Artifact checksum mismatch

An artifact does not match its manifest digest.

Class: `artifact`. Exit code: 1.

## `PP4002` --- Missing published artifact

An expected artifact was not produced by the build pipeline.

Class: `artifact`. Exit code: 1.

## `PP4003` --- Emitter inputs digest drift

The emitter-semantics ID does not match the digest of the declared emitter inputs.

Class: `artifact`. Exit code: 1.

## `PP4004` --- Invalid manifest structure

A build or verification manifest fails schema validation.

Class: `artifact`. Exit code: 1.

## `PP4101` --- Generated stdlib runtime mismatch

The generated prism-stdlib runtime differs from its authoritative LexLean, Lean, LCNF, or fixed-point evidence.

Class: `artifact`. Exit code: 1.

## `PP4102` --- Generated application code mismatch

Generated application code contains unbound, handwritten, duplicated, or stale application behavior.

Class: `artifact`. Exit code: 1.

## `PP4103` --- Cargo package publication failure

A generated Cargo package cannot be packaged, consumed, reproduced, or published under its declared exact identity.

Class: `artifact`. Exit code: 1.

## `PP4104` --- Archive publication failure

A release archive or its bound checksum/provenance could not be atomically staged or published.

Class: `artifact`. Exit code: 1.

## `PP4105` --- Browser publication failure

The generated browser application differs from its acceptance evidence or could not be published at the registered URL.

Class: `artifact`. Exit code: 1.

## `PP5001` --- Lean elaboration failure

Generated Lean declarations failed formal elaboration.

Class: `oracle`. Exit code: 1.

## `PP5002` --- leanchecker replay failure

leanchecker kernel replay rejected a compiled module environment.

Class: `oracle`. Exit code: 1.

## `PP5003` --- Unaudited Lean axiom

A PrismPM-owned theorem depends on unapproved external axioms.

Class: `oracle`. Exit code: 1.

## `PP5004` --- lean4-prod export failure

lean4-prod failed to export LCNF for registered runtime roots.

Class: `oracle`. Exit code: 1.

## `PP5005` --- Rust validator compilation error

Generated Rust code failed to compile under the pinned toolchain.

Class: `oracle`. Exit code: 1.

## `PP5006` --- Validator execution mismatch or panic

A generated Rust validator panicked or produced an inconsistent result during property testing.

Class: `oracle`. Exit code: 1.

## `PP5007` --- Child process resource limit exceeded

A verification child process exceeded its fixed timeout or captured-output limit.

Class: `oracle`. Exit code: 1.

## `PP5008` --- Required verification executable unavailable

A pinned verification executable or source package is missing or unreadable.

Class: `oracle`. Exit code: 1.

## `PP5101` --- Guest ABI disagreement

A generated Hologram guest does not implement the exact core-wasm@1 pointer/length ABI.

Class: `oracle`. Exit code: 1.

## `PP5102` --- Forbidden guest import

A generated Core-Wasm guest has a nonempty import table.

Class: `oracle`. Exit code: 1.

## `PP5103` --- Guest export disagreement

A generated Core-Wasm guest is missing or has the wrong type for memory, holo_alloc, or holo_run.

Class: `oracle`. Exit code: 1.

## `PP5104` --- Guest memory contract failure

A guest allocation, pointer, length, cap, overlap, growth, trap, or reset behavior violates core-wasm@1.

Class: `oracle`. Exit code: 1.

## `PP5201` --- Portable View bundle disagreement

A HOLOVIEW bundle has invalid magic, version, entry, path order, limits, framing, or trailing bytes.

Class: `oracle`. Exit code: 1.

## `PP5202` --- View model disagreement

An evaluated View value does not match the typed View model in the authoritative application.

Class: `oracle`. Exit code: 1.

## `PP5203` --- View projection disagreement

A Hologram or browser View projection differs from its canonical modeled projection.

Class: `oracle`. Exit code: 1.

## `PP5204` --- View intent disagreement

A View action, validation, result, error, focus, or live-region trace differs from the modeled intent.

Class: `oracle`. Exit code: 1.

## `PP5205` --- View surface disagreement

A runtime surface is missing, unsupported, or attached with a contract different from the manifest.

Class: `oracle`. Exit code: 1.

## `PP5301` --- Upstream Hologram oracle disagreement

The pinned independent Hologram loader, planner, executor, or View encoder disagrees with Prism evidence.

Class: `oracle`. Exit code: 1.

## `PP6001` --- Reference model identity drift

The reference repository model or lock is not byte-identical to the authoritative PrismPM getting-started source closure.

Class: `acceptance`. Exit code: 1.

## `PP6002` --- Incomplete application acceptance

A required application acceptance gate is missing, skipped, stale, conditional, failed, or not bound to current artifacts.

Class: `acceptance`. Exit code: 1.

## `PP6003` --- Application execution disagreement

Generated Rust, Wasm, Hologram, or browser execution differs over the authoritative acceptance corpus.

Class: `acceptance`. Exit code: 1.

## `PP6004` --- Ecosystem release incomplete

PrismPM completion was requested before all registry, archive, reference repository, Pages, CI, and release identities were verified.

Class: `acceptance`. Exit code: 1.

## `PP8001` --- Unsafe filesystem access rejected

An operation selected an unsafe path, filesystem object, existing destination, or ownership boundary, or attempted to escape the project root.

Class: `security`. Exit code: 1.

## `PP8002` --- Disallowed network access

A network operation was attempted during an offline verification stage.

Class: `security`. Exit code: 1.

## `PP1101` --- Missing or invalid immutable lock

A required standards, SDK, dependency, adapter, or deployment lock is missing, noncanonical, incomplete, or mutable.

Class: `configuration`. Exit code: 2.

## `PP2101` --- Invalid production system closure

The production-system graph has an absent, duplicate, dangling, cyclic, incompatible, unsatisfied, or incomplete modeled value.

Class: `model`. Exit code: 3.

## `PP2102` --- Invalid environment or secret binding

An environment value is undeclared or ill-typed, changes immutable behavior, or a secret value entered a model or artifact.

Class: `model`. Exit code: 3.

## `PP2103` --- Unsafe migration or lifecycle transition

A migration, rollback, recovery, drift, retirement, or compatibility transition is absent, unordered, unsafe, or unauthorized.

Class: `model`. Exit code: 3.

## `PP5401` --- Authority resolution failure

An authority source, edition, immutable identity, license, signature, supersession rule, or lawful acquisition binding could not be resolved.

Class: `authority`. Exit code: 4.

## `PP5402` --- Authority bytes or signature mismatch

Acquired authority or oracle bytes disagree with their locked digest or published signature.

Class: `authority`. Exit code: 4.

## `PP5403` --- Oracle invocation failure

A digest-bound oracle was missing, escaped isolation, exceeded a bound, returned an unknown exit, or validated a different subject.

Class: `authority`. Exit code: 4.

## `PP5404` --- Insufficient standards coverage

A standards claim exceeds the exact requirements covered by its authoritative schema, corpus, conformance tool, or reference implementation.

Class: `authority`. Exit code: 4.

## `PP5405` --- Stale authority or oracle evidence

Authority, oracle, vulnerability, license, or target evidence is stale for the declared policy or uses a different edition.

Class: `authority`. Exit code: 4.

## `PP6101` --- Unverified product release

A build, publication, promotion, run, or deployment selected a release without complete verification bound to its root digest.

Class: `artifact`. Exit code: 5.

## `PP6201` --- Registry operation failure

An OCI registry authentication, upload, download, discovery, referrer, concurrency, or remote-byte verification operation failed.

Class: `registry`. Exit code: 6.

## `PP6301` --- Invalid OCI release graph

An OCI descriptor graph has a digest, size, media type, subject, role, closure, duplication, dangling edge, or cycle disagreement.

Class: `artifact`. Exit code: 5.

## `PP6401` --- Supply-chain policy rejection

A signature, signer, builder, provenance, SBOM, vulnerability, license, freshness, promotion, or trust-policy requirement was not met.

Class: `policy`. Exit code: 7.

## `PP7001` --- Target capability mismatch

The selected target or adapter cannot satisfy the model, API version, capability, identity, storage, isolation, or policy contract.

Class: `target`. Exit code: 8.

## `PP7101` --- Deployment plan invalid or stale

A deployment plan is incomplete, noncanonical, expired, destructive without policy, or bound to a different observed target state.

Class: `deployment`. Exit code: 9.

## `PP7901` --- Backup or recovery failure

A modeled backup, integrity check, clean-target restore, acceptance replay, or RPO/RTO bound failed or lacks digest-bound evidence.

Class: `recovery`. Exit code: 9.

## `PP7201` --- Deployment health or SLO failure

Startup, readiness, liveness, contract, telemetry, acceptance, or bounded SLO evaluation failed for the deployed digest.

Class: `health`. Exit code: 10.

## `PP7301` --- Deployment drift detected

Observed target state differs from both the desired and last-applied identities or contains unmanaged behavior.

Class: `drift`. Exit code: 11.

## `PP7401` --- Rollback safety failure

The requested prior release or data state is unavailable, unaccepted, incompatible, or unsafe to restore without a forward recovery.

Class: `rollback`. Exit code: 12.

## `PP7501` --- Operation authorization failure

Publication, deployment, rollback, secret access, migration, or destruction lacks the exact modeled identity and environment authorization.

Class: `authorization`. Exit code: 13.

## `PP7601` --- Operation timeout or resource exhaustion

A controller, adapter, oracle, registry, runtime, or recovery operation exceeded its modeled time, output, memory, storage, or concurrency bound.

Class: `resource`. Exit code: 14.

## `PP7701` --- Retirement or destruction refused

Retirement or destruction is not separately requested, retention-safe, state-bound, and authorized for the selected environment.

Class: `authorization`. Exit code: 13.

## `PP7801` --- Secret material detected

A source, lock, plan, artifact, OCI blob, BOM, attestation, log, or evidence value contains prohibited secret material.

Class: `policy`. Exit code: 7.

## `PP9001` --- Internal invariant failure

An internal compiler or runtime invariant was violated.

Class: `internal`. Exit code: 101.

## Browser journal diagnostics

JournalAdapterError codes are registered in model/browser-diagnostics.toml and owned by DK-12. Known propagated BrowserEffectError codes retain the SPEC section 12.1 namespace. Private defensive-boundary tests do not claim those invalid internal values are public journeys.

| Code | Meaning |
|---|---|
| `author-mismatch` | The signed event author does not match the actual P-256 public-key principal. |
| `event-id-mismatch` | The event ID differs from the SHA-256 digest of its generated domain-separated signing preimage. |
| `generated-execution-failed` | The trusted generated journal guest could not execute; no candidate state is promoted. |
| `invalid-generated-module` | The trusted bootstrap did not provide a WebAssembly module with the required journal exports. |
| `invalid-generated-output` | The generated output framing, bounds or completion binding is invalid. |
| `invalid-hash` | A trusted-boundary content reference is not a canonical lowercase SHA-256 identifier. |
| `invalid-input` | The public journal call has an invalid argument count or head name. |
| `invalid-length` | An internal framing length is not an unsigned integer representable in 24 bits. |
| `journal-busy` | The local host already owns two outstanding append/refresh operations; additional work is rejected before input capture or effects. |
| `model-rejected` | The generated model rejected the candidate; detail carries only its bounded rejection bytes. |
| `object-corrupt` | Retained event or head bytes differ from their exact content identifier. |
| `object-missing` | Complete authenticated replay cannot fetch a referenced retained event object. |
| `replay-required` | A failed or uncertain outcome requires complete authenticated replay before another write. |
| `request-limit` | An internal guest request exceeds the modeled allocation cap before guest execution. |
| `signature-invalid` | Actual browser cryptography rejects the context-bound event signature. |
| `storage-outcome-unknown` | The trusted storage adapter returned an unrecognized exception; its message, payload and cause are discarded and replay is required. |
| `storage-rejected` | The generated completion bound an actual storage conflict or failure; detail carries only the modeled status byte. |
| `storage-result-mismatch` | The real storage call returned a head other than the exact candidate; no promotion or retry occurs before replay. |

## CommandAdapterError diagnostics

Registered in model/browser-adapter-diagnostics.toml; owned by DK-13. JournalAdapterError and BrowserEffectError remain separate propagated namespaces. Negative-only guard tests are not public journeys.

| Code | Meaning |
|---|---|
| `adapter-busy` | Two operations are already owned; additional work is rejected before intent capture. |
| `adapter-closed` | The adapter is closed; captured or late work cannot expose a result or begin a new effect. |
| `command-rejected` | The generated Command rejected the operation; detail is exactly its registered one-byte failure. |
| `commit-outcome-unknown` | Close occurred during an already started durable write; no rollback is claimed and reopening must replay. |
| `generated-execution-failed` | The trusted generated guest trapped or could not execute; no successful result is exposed. |
| `identity-missing` | The private storage namespace has no persisted identity. |
| `invalid-effect-result` | A trusted-boundary hash is not a canonical lowercase SHA-256 identifier. |
| `invalid-generated-module` | Trusted bootstrap did not provide the expected import-free generated module and exports. |
| `invalid-generated-output` | Generated output framing, bounds or required fields are invalid. |
| `invalid-input` | The call has an invalid argument count, closed intent shape, type or byte bound. |
| `refresh-required` | A failed or uncertain command requires explicit complete authenticated replay before another write. |
| `request-limit` | The private guest request exceeds the exact modeled allocation cap before guest execution. |
| `storage-outcome-unknown` | An unknown storage exception is replaced by a stable code without retaining its message, payload or cause. |

The `command-rejected` detail is the exact generated byte:

| Byte | Modeled rejection |
|---|---|
| 1 | `BadEncoding` |
| 2 | `InvalidCommand` |
| 3 | `InvalidHead` |
| 4 | `WorkspaceMismatch` |
| 5 | `EventLimit` |
| 6 | `WrongPhase` |
| 7 | `CorrelationMismatch` |
| 8 | `StaleHead` |
| 9 | `EffectMismatch` |
| 10 | `InvalidResult` |
| 11 | `UnknownOperation` |
| 12 | `InvalidPending` |

## QueryAdapterError diagnostics

Registered in model/browser-adapter-diagnostics.toml; owned by DK-14. JournalAdapterError and BrowserEffectError remain separate propagated namespaces. Negative-only guard tests are not public journeys.

| Code | Meaning |
|---|---|
| `adapter-busy` | Two operations are already owned; additional work is rejected before intent capture. |
| `adapter-closed` | The adapter is closed; captured or late work cannot expose a result or begin a new effect. |
| `generated-execution-failed` | The trusted generated guest trapped or could not execute; no successful result is exposed. |
| `identity-missing` | The private storage namespace has no persisted identity. |
| `invalid-effect-result` | A trusted-boundary hash is not a canonical lowercase SHA-256 identifier. |
| `invalid-generated-module` | Trusted bootstrap did not provide the expected import-free generated module and exports. |
| `invalid-generated-output` | Generated output framing, bounds or required fields are invalid. |
| `invalid-input` | The call has an invalid argument count, closed intent shape, type or byte bound. |
| `query-context-changed` | Authenticated head or state changed across asynchronous hashing; no stale page is disclosed. |
| `query-rejected` | The generated Query rejected admission or cursor use; detail is exactly its registered one-byte failure. |
| `request-limit` | The private guest request exceeds the exact modeled allocation cap before guest execution. |
| `storage-outcome-unknown` | An unknown storage exception is replaced by a stable code without retaining its message, payload or cause. |

The `query-rejected` detail is the exact generated byte:

| Byte | Modeled rejection |
|---|---|
| 1 | `BadEncoding` |
| 2 | `InvalidHead` |
| 3 | `InvalidState` |
| 4 | `InvalidContext` |
| 5 | `WrongWorkspace` |
| 6 | `NotAdmitted` |
| 7 | `InvalidCursor` |
| 8 | `StaleCursor` |
| 9 | `CursorSession` |
| 10 | `CursorPrincipal` |
| 11 | `CursorTable` |
| 12 | `CursorRange` |
| 13 | `UnknownTable` |

## ViewHostError diagnostics

Registered separately in model/browser-view-diagnostics.toml; owned by DK-16. Private defensive guards are distinct from public journeys. Journal and Command/Query namespaces are unchanged.

| Code | Meaning |
|---|---|
| `generated-execution-failed` | The private generated View guest trapped or failed execution; no successful transition is exposed. |
| `host-unavailable` | Bootstrap or private execution failed; the public boundary exposes no exception payload and an opened View terminalizes and clears its display. |
| `invalid-effect-result` | A private admitted Query result has invalid fields or bounds and cannot become generated presentation. |
| `invalid-generated-module` | Trusted bootstrap did not provide the exact import-free guest contract. |
| `invalid-generated-output` | A private generated transition, effect or presentation violates its registered framing or bounds. |
| `invalid-input` | A public dispatch or close call has invalid arity, type or byte bounds; no input is asynchronously recaptured. |
| `invalid-labels` | The modeled labels artifact is not the closed canonical UTF-8 JSON schema within its exact byte limits. |
| `invalid-root` | The private renderer did not receive a connected same-realm HTML element. |
| `model-rejected` | The generated View rejected the operation with exactly one registered rejection byte; no replacement state or effect is accepted. |
| `request-limit` | The private generated View input exceeds its fixed allocation cap before guest execution. |
| `view-closed` | The View is terminal; public dispatch cannot begin work or restore presentation. |

The model-rejected detail is the exact generated byte:

| Byte | Modeled rejection |
|---|---|
| 1 | `BadEncoding` |
| 2 | `InvalidState` |
| 3 | `InvalidIntent` |
| 4 | `Busy` |
| 5 | `ReplayRequired` |
| 6 | `Closed` |
| 7 | `CounterExhausted` |
| 8 | `NoSelection` |
| 9 | `NoNextPage` |
| 10 | `WrongPhase` |
| 11 | `CorrelationMismatch` |
| 12 | `InvalidOutcome` |
| 13 | `InvalidPage` |
| 14 | `UnknownOperation` |
