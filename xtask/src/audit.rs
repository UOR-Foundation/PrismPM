...
pub fn audit_no_handwritten_lean(root: &Path) -> Result<(), Fail> {
    for entry in walkdir::WalkDir::new(root).into_iter().flatten() {
        ...
        if rel.starts_with(".prism") || rel.starts_with(".lexlean") || rel.starts_with("target") || rel.contains("expected") {
            continue;
        }
        if rel.ends_with(".lean") {
            return Err(format!("no-handwritten-lean audit failed: found {}", path.display()).into());
        }
...
pub fn audit_no_unsafe(root: &Path) -> Result<(), Fail> {
    for entry in walkdir::WalkDir::new(root.join("crates")).into_iter().flatten() {
        if !entry.file_type().is_file() {
            continue;
        }
        let path = entry.path();
        if path.extension().is_some_and(|ext| ext == "rs") {
            ...
            if text.contains("unsafe ") && !path.to_string_lossy().contains("tests") {
                return Err(format!("unsafe audit failed: found unsafe in {}", path.display()).into());
            }
        }
    }
    Ok(())
...
pub fn audit_shipped(root: &Path) -> Result<(), Fail> {
    ...
    for cr in internal_crates {
        ...
        if !content.contains("publish = false") {
            return Err(format!("{}: internal crate must have publish = false", manifest_path.display()).into());
        }
    }
    Ok(())
}
    ...
    for entry in walkdir::WalkDir::new(root.join("crates")).into_iter().flatten() {
        ...
        if path.extension().is_some_and(|ext| ext == "rs") {
            ...
            for word in text.split(|c: char| !c.is_alphanumeric()) {
                if word.starts_with("PP") && word.len() == 6 && word.chars().skip(2).all(|c| c.is_ascii_digit()) {
                    if !registered.contains(word) {
                        return Err(format!("{}: references unregistered error code `{word}`", path.display()).into());
                    }
                }
            }
        }
