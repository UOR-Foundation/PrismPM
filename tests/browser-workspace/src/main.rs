        ...
        [mode, input, output, licenses] if mode == "generate" || mode == "generate-wasm" => {
            ...
            let package = if mode == "generate-wasm" {
                generate_core_wasm_package(
                    &module,
                    &CoreWasmSpec {
                        crate_name: "browser-workspace-wasm-probe".into(),
                        entry: "reduceWorkspaceBytes".into(),
                        export_name: "holo_run".into(),
                        input_allocation_cap: 1_104_664,
                        output_allocation_cap: 1_100_428,
                        maximum_pages: 512,
                ...
                generate_cargo_package(
                    &module,
                    &CargoPackageSpec {
                        name: "browser-workspace-core-probe".into(),
                        version: "0.1.0".into(),
                        description:
