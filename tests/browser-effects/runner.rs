use browser_effects_core_probe::{effectWireBytes, fixtureEchoBytes};
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    if args.len() == 3 && args[0] == "--binary" {
        ...
        for _ in 0..2 {
            let actual = effectWireBytes(input.clone()).map_err(|error| format!("{error:?}"))?;
            assert!(actual == expected, "binary native output mismatch");
        }
        println!("PASS binary complete effects vector twice");
        return Ok(());
    }
    if args.len() != 1 {
    ...
    for line in fs::read_to_string(&args[0])?.lines() {
        ...
        for _ in 0..2 {
            let actual = (if fields[0].starts_with("BrowserGuest") {
                Ok(fixtureEchoBytes(input.clone()))
            } else {
                effectWireBytes(input.clone())
            })
    ...
    println!("PASS {count} complete effects vectors twice");
    Ok(())
}
