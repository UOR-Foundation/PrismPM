        ...
        [mode, input, output, licenses] if ["native", "wasm"].contains(&mode.as_str()) => {
            ...
            let package = if mode == "native" {
                generate_cargo_package(
                    &module,
                    &CargoPackageSpec {
                        name: "browser-budget-core-probe".into(),
                        version: "0.1.0".into(),
                        description: "Generated internal budget verification fixture".into(),
                ...
                generate_core_wasm_package(
                    &module,
                    &CoreWasmSpec {
                        crate_name: "browser-budget-wire-probe".into(),
                        entry: "budgetProbeBytes".into(),
