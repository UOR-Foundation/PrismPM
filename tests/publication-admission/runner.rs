use publication_admission_core_probe::publicationWireBytes;
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    if args.len() == 3 && args[0] == "--binary" {
        ...
        for _ in 0..2 {
            let actual =
                publicationWireBytes(input.clone()).map_err(|error| format!("{error:?}"))?;
            assert!(actual == expected, "binary native output mismatch");
        }
        println!("PASS binary complete publication vector twice");
        return Ok(());
    }
    if args.len() != 1 {
    ...
    for line in fs::read_to_string(&args[0])?.lines() {
        ...
        for _ in 0..2 {
            let actual = publicationWireBytes(input.clone())
    ...
    println!("PASS {count} complete publication vectors twice");
    Ok(())
}
