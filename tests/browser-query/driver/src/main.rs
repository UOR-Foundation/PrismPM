...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    match args.as_slice() {
        ...
        [mode, input, output, licenses]
            if mode == "generate"
                || mode == "generate-wasm"
                || mode == "generate-journal-wasm"
                || mode == "generate-probe-wasm" =>
        {
            ...
            let package = if mode != "generate" {
                ...
                let probe = mode == "generate-probe-wasm";
                generate_core_wasm_package(
                    &module,
                    &CoreWasmSpec {
                        crate_name: "browser-workspace-query-wasm-probe".into(),
                        entry: if journal {
                            "workspaceJournalBytes"
                        } else if probe {
                            "queryDecodeProbeBytes"
                        } else {
                            "workspaceQueryBytes"
                        }
                        .into(),
                        export_name: "holo_run".into(),
                        input_allocation_cap: if journal {
                            1_235_980
                        } else if probe {
                            2
                        } else {
                        ...
                        output_allocation_cap: if journal {
                            1_166_008
                        } else if probe {
                            2
                        } else {
                ...
                generate_cargo_package(
                    &module,
                    &CargoPackageSpec {
                        name: "browser-workspace-query-core-probe".into(),
                        version: "0.1.0".into(),
                        description:
