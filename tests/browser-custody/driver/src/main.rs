...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    match args.as_slice() {
        ...
        [mode, input, output, licenses] if ["native", "wasm"].contains(&mode.as_str()) => {
            ...
            let package = if mode == "native" {
                generate_cargo_package(
                    &module,
                    &CargoPackageSpec {
                        name: "browser-custody-core-probe".into(),
                        version: "0.1.0".into(),
                        description: "Generated internal credential custody verification fixture"
                            .into(),
                ...
                generate_core_wasm_package(
                    &module,
                    &CoreWasmSpec {
                        crate_name: "browser-custody-wire-probe".into(),
                        entry: "custodyWireBytes".into(),
                        export_name: "holo_run".into(),
                        input_allocation_cap: 2_097_152,
                        output_allocation_cap: 65_536,
                        maximum_pages: 2_048,
        ...
        _ => {
            return Err(
                "expected verify PROJECT or native|wasm IR ABSENT_OUTPUT LICENSE_ROOT".into(),
            )
        }
    }
    Ok(())
