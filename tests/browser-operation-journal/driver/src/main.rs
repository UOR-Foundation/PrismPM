...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    match args.as_slice() {
        [mode, project] if mode == "check" => {
            ...
            let checked = engine.check(CheckRequest {
                selection: Selection::All,
            })?;
            println!(
                "{}",
                serde_json::json!({"modules": checked.units.keys().collect::<Vec<_>>(), "source_id": checked.source_id.to_string()})
            );
        }
        ...
        [mode, input, output, licenses]
            if ["native", "wasm", "guest", "journal", "partition"].contains(&mode.as_str()) =>
        {
            ...
            let package = if mode == "native" {
                generate_cargo_package(
                    &module,
                    &CargoPackageSpec {
                        name: "browser-operation-journal-core-probe".into(),
                        version: "0.1.0".into(),
                ...
                generate_core_wasm_package(
                    &module,
                    &CoreWasmSpec {
                        crate_name: format!("browser-operation-journal-{label}-probe"),
                        entry: match mode.as_str() {
                            "guest" => "fixtureEchoBytes",
                            "journal" => "journalWireBytes",
                            "partition" => "journalPartitionBytes",
                            _ => "effectWireBytes",
                        }
                        .into(),
