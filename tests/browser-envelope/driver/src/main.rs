        ...
        [mode, input, output, licenses] if mode == "generate" || mode == "generate-wasm" => {
            ...
            let package = if mode == "generate-wasm" {
                generate_core_wasm_package(
                    &module,
                    &CoreWasmSpec {
                        crate_name: "browser-workspace-envelope-wasm-probe".into(),
                        entry: "workspaceEnvelopeBytes".into(),
                        export_name: "holo_run".into(),
                        input_allocation_cap: 4_364,
                        output_allocation_cap: 4_364,
                        maximum_pages: 32,
                ...
                generate_cargo_package(
                    &module,
                    &CargoPackageSpec {
                        name: "browser-workspace-envelope-core-probe".into(),
                        version: "0.1.0".into(),
                        description:
