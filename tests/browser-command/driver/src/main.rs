...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    match args.as_slice() {
        ...
        [mode, input, output, licenses]
            if mode == "generate" || mode == "generate-wasm" || mode == "generate-journal-wasm" =>
        {
            ...
            let package = if mode != "generate" {
                let journal = mode == "generate-journal-wasm";
                generate_core_wasm_package(
                    &module,
                    &CoreWasmSpec {
                        crate_name: "browser-workspace-command-wasm-probe".into(),
                        entry: if journal {
                        ...
                        input_allocation_cap: if journal { 1_235_980 } else { 139_873 },
                        output_allocation_cap: if journal { 1_166_008 } else { 74_243 },
                        maximum_pages: if journal { 640 } else { 64 },
                ...
                generate_cargo_package(
                    &module,
                    &CargoPackageSpec {
                        name: "browser-workspace-command-core-probe".into(),
                        version: "0.1.0".into(),
                        description:
